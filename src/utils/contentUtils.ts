...
export function getResourceDescription(resourceName: string, providerName: string): string {
  ...
  const descriptions: Record<string, string> = {
    ...
    s3_bucket:
      "An S3 bucket is a container for storing objects in Amazon S3. You can use buckets to store and serve files, host static websites, or as a destination for logs.",
    instance:
    ...
    subnet:
      "A subnet is a range of IP addresses in your VPC that can be used to isolate resources within your network.",
    security_group:
 ...
 */
export function generateTemplateExample(resourceName: string, resourceType?: string, example?: string): string {
  ...
  const resourceTypeToUse =
    resourceType || (resourceName.includes("_") ? resourceName.substring(resourceName.indexOf("_") + 1) : resourceName);
